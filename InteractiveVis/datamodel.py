...
from components.ConfusionMatrix import display_confusion_matrix_metrics
...
class Model:
    ...
    def set_model(self, new_model_name):
        ...
        self.inference_engine = InferenceEngine(self.selected_model)
    ...
    def get_performance_metrics(self, images, labels, class_names=['CN', 'MCI', 'AD']):
        """
        Get model performance metrics including confusion matrix and ROC curves.
        
        Args:
            images (np.ndarray): Input images for evaluation
        ...
        if self.inference_engine is None:
            raise ValueError("Model must be loaded before evaluating performance")
        return self.inference_engine.evaluate_performance(images, labels, class_names)
    ...
    def __init__(self):
        ...
        self.inference_engine = None # inference engine for predictions and evaluation
        self.true_labels = [] # true labels for confusion matrix
        self.predicted_labels = [] # predicted labels for confusion matrix
    ...
    def analyze_model_performance(self):
        """
        Analyze model performance using confusion matrix and related metrics.
        """
        if len(self.true_labels) > 0 and len(self.predicted_labels) > 0:
            ...
            st.warning("No prediction data available for analysis. Please run some predictions first.")

    def add_prediction_result(self, true_label, predicted_label):
        """
        Add a prediction result to the analysis data.
        
        Args:
            true_label (str): True class label
